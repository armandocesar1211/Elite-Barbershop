...
const gallery = [{src:'/assets/gallery-corte-classico.png',alt:'Corte clássico finalizado na Elite Barbershop',label:'Corte com assinatura'},{src:'/assets/gallery-platinado.png',alt:'Corte platinado finalizado na Elite Barbershop',label:'Estilo em cada detalhe'},{src:'/assets/gallery-ambiente.png',alt:'Cliente na cadeira da Elite Barbershop',label:'A experiência Elite'},{src:'/assets/gallery-acabamento.png',alt:'Acabamento de barba na Elite Barbershop',label:'Precisão no acabamento'}];
export default function Home() {
 ...
 return <S.Page>
   ...
   <S.Gallery id="galeria"><ScrollReveal><S.GalleryTitle><p>03 — GALERIA</p><h2>O PADRÃO<br/>É <i>ELITE.</i></h2><span>Cortes, detalhes e momentos reais da nossa casa.</span></S.GalleryTitle></ScrollReveal><S.GalleryCarousel><InfiniteSlider aria-label="Galeria de cortes e momentos da Elite Barbershop">{gallery.map(item=><S.GalleryTile key={item.src}><img src={item.src} alt={item.alt}/><figcaption>{item.label}</figcaption></S.GalleryTile>)}</InfiniteSlider><InfiniteSlider reverse aria-label="Segunda faixa da galeria de cortes e momentos">{gallery.map(item=><S.GalleryTile key={item.src}><img src={item.src} alt=""/><figcaption>{item.label}</figcaption></S.GalleryTile>)}</InfiniteSlider></S.GalleryCarousel></S.Gallery>
